...
public:
	...
	struct AAA_Node
	{
		...
		void change_subtree(T delt, int type)//1:cover_tag;	2:add_tag
		{
			if(type == 1)
			{
				data_subtree = size_subtree * delt;
				tag_subtree = delt;
				tag_subtree_type = 1;
			}else//type == 2
			{
				...
				{
					data_subtree = data_subtree & (size_subtree * delt);
					tag_subtree = delt;
					tag_subtree_type = 2;
				}else
				{
	...
	void access(AAA_Node* x)
	{
		solve_tag(x);
		for (AAA_Node* v = 0; x; v = x, x = x -> fa)
		{
			AAA_Node *tmp = x;
			x -> push_down();
...
int main( )
{
	...
	T.make_root(1);
	T.access(T.get(n));
	for(int i = n - 2; i >= 1; i -= 6){
		T.access(T.get(i));
	}
	T.cover_subtree(1, 1);
	int ans1=T.ask_subtree(n - 10);
	for(int i = n - 9; i <= n; ++i)
		T.get(i) -> push_down();
	printf("ans1 = %d, 4->data = %d\n",ans1,T.get(n)->val);
	return 0;
}
